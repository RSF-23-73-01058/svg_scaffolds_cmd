			...
			if svg_string.startswith("<path class='bond"):
				...
				if ( any(cc_bonds == bond_id for cc_bonds in cc_bonds) ):
					...
					if start.count("L")==1 and start.count("Z")==0 :
						...
						if typical_bond_length == 0 or min( ( 1 - 874/typical_bond_length ), ( 1 - 600/typical_bond_length ) ) > min( distance_diff_typ1, distance_diff_typ1 ): typical_bond_length = distance
