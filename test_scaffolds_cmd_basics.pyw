...
def process_input(data):
	...
	for row in data_base['data']:
		...
		if non_generic == 0:
			scaff = MurckoScaffold.GetScaffoldForMol(MurckoScaffold.MakeScaffoldGeneric(scaff))
		scaff_smiles = Chem.MolToSmiles(scaff)
		if scaff_smiles != '' and scaff_smiles != 'NONE':
			for bond in scaff.GetBonds():
				anum1 = bond.GetBeginAtom().GetAtomicNum()
				anum2 = bond.GetEndAtom().GetAtomicNum()
				if anum1 == 6 and anum2 == 6:
					n_cc_bonds += 1
		if n_cc_bonds > max_cc_bonds:
			max_cc_bonds = n_cc_bonds
		if scaff_smiles == '': scaff_smiles = '*'
		if scaff_smiles == 'NONE': scaff_smiles = '[*-]'
		scaff_list.append(scaff_smiles)
		smiles_list.append(smiles)
	##Gather meta data
	#Number of processed structures
	n_structures = n_structures - failed_mol - failed_mw - failed_bond
	if n_structures == 0:
	...
	meta_data = {'processed': n_structures, 'repeats': smiles_repeats, 'failed_parse': failed_mol, 'failed_mw': failed_mw, 'failed_bond': failed_bond}
	#Count scaffolds
	scaffold_counted = Counter(scaff_list).most_common()
	...
	selected_description = {'number': scaffold_limit, 'scaffold_fraction': fraction_selected_scaffold, 'structure_fraction_forSelected':  fraction_selected_structure}
	#Some vars to process the scaffolds
	scaffold_id = 0
	potential_size = int(max_cc_bonds) * initial_score
	...
	for scaff_count in scaffold_counted:
		...
		if scaff_count[0] == '[*-]': scaff_mol.GetAtoms()[0].SetProp('atomLabel', "UNKNOWN")
		...
		for i, at in enumerate(atoms):
			atom_cols[at] = color
		#Draw
		drawer = rdMolDraw2D.MolDraw2DSVG(potential_size, potential_size)
		drawer.drawOptions().fixedFontSize = 30
		drawer.drawOptions().padding = 0
		drawer.drawOptions().fixedBondLength = initial_score
		...
		#New header
		groupID = "<svg xmlns='http://www.w3.org/2000/svg' id = '" + str(scaffold_id) + "'  viewBox = '0 0 200 200'  width = '200' height = '200' > <g id = '" + str(scaffold_id) + "' opacity = '0.7' class = 'thing'>"
		svg_text = re.sub(r'(?m)<\?xml.*?</rect>', groupID, svg_text_original, flags=re.S)
		#Prepare to find the largest molecule out there:
		atom_x_min =  potential_size + 10
		atom_x_max = -potential_size - 10
		atom_y_min =  potential_size + 10
		atom_y_max = -potential_size - 10
		...
		for i, svg_string in enumerate(svg_strings):
			if svg_string.startswith(('<ellipse', '<circle')):
				...
				#Modify elements
				svg_string = svg_string.replace(';stroke:#FFFFFF;stroke-width:1.0px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1', '')
		...
		data_to_transform.append({'id': scaffold_id, 'score': score, 'bbox_scored': bbox_scored, 'status': scaffold_status,
							'size': scaff_dimension_scored, 'potential_size': potential_size, 'svg': svg_text})
	##Resize SVGs
	#Sort the SVGs
	...
	for item in data_to_transform:
		...
		#Append
		data_to_json.append({'id': item['id'], 'min_x': bbox_scaled[0], 'min_y': bbox_scaled[1], 'score': item['score'], 'length': length_scaled, 'height': height_scaled,  'svg_thing': str(base64.b64encode(zlib.compress(svg_text_transformed.encode('utf-8'))))})
...
try:
	...
	result = '{"news": "something is wrong"}'
